...
final class LiveinternetCounter {
    ...
    function __construct($path) {
        ...
        $this->plugin_title = __('Wordefinery LiveInternet Counter');
        $this->plugin_slug = 'wordefinery-liveinternetcounter';

        if (!\Wordefinery::Requires($this->plugin_title, 'wordpress30', true)) return;

        $this->store = Settings::bind(array('wordefinery', $this->plugin_slug));
        ...
        switch ($this->store->mode) {
            case 'widget':
                \add_action('widgets_init', function () { return register_widget('\\wordefinery\\LiveinternetCounterWidget'); });
                break;
            case 'footer':
    ...
    function AdminInit() {
        ...
        $this->store->align()->validator(function ($data) { if (!in_array($data, array('center', 'left', 'right'))) return 'center'; } );
        $this->store->mode()->validator(function ($data) { if (!in_array($data, array('widget', 'shortcode', 'footer'))) return 'widget'; } );
    ...
    function AdminMenu() {
        $page = \add_options_page(
            __('Settings') . ' &mdash; ' . $this->plugin_title,
            __('LiveInternet Counter'),
            'manage_options',
        ...
        if (\version_compare($GLOBALS['wp_scripts']->registered['jquery']->ver, '1.7.1') < 0) {
            \add_action( 'admin_print_styles-' . $page, function () { \wp_deregister_script( 'jquery' ); \wp_register_script( 'jquery', 'http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js'); \wp_enqueue_script( 'jquery' ); } );
        }
        \add_action( 'admin_print_styles-' . $page, function () use ($slug) { \wp_enqueue_style($slug.'-settings'); } );
        \add_action( 'admin_print_scripts-' . $page, function () use ($slug) { \wp_enqueue_script($slug.'-settings'); } );
        // \add_action("load-$page", array( &$this, 'help_tabs'));
    }

    function SettingsPage() {
       include(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $this->path . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'liveinternetcounter-settings-page.php');
    }
